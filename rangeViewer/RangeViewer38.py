...
# Bisecting directions with exact (row, col) deltas (kept for reference, though not used in new Mist/Shadow)
...
def get_hex_direction(pos, target):
    ...
    if dr == 0:
        ...
        # Approximate vertical (not exact in hex, but closest match)
            ...
            if angle in [90, 270]:
                ...
                for step in range(1, steps + 1):
                    ...
                    if (0 <= new_r < grid.rows and 0 <= new_c < grid.cols and 
                        grid.hex_distance(pos, new_pos) <= dist and grid.grid[new_r][new_c]["accessible"]):
                        line.append(new_pos)
                    else:
                        break  # Stop at obstruction or out of bounds
            else:
