...
# Define bisecting directions (approximated for hex grid, 30° increments)
BISECTING_DIRECTIONS = [
    (1, 1, -2),  # Approx 30°
    (0, -1, 1),  # 90° (already in DIRECTIONS, but reused for clarity)
    (-1, -2, 1), # Approx 150°
    (-1, 0, 1),  # 210° (already in DIRECTIONS)
    (0, 2, -2),  # Approx 270°
    (1, 1, 0),   # Approx 330°
]
...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    ...
    if pattern == "Line of Sight":
        ...
        # Get all accessible hexes within range
        all_hexes = grid.get_movement_range(pos, dist)
            ...
            for hex_pos in line:
                if grid.hex_distance(pos, hex_pos) <= dist:
                    ...
                    if not grid.grid[hex_pos[0]][hex_pos[1]]["accessible"]:
                        break
        # Define range using bisecting lines, stopping at obstructions
        range_set = set()
        for dir in BISECTING_DIRECTIONS:
            line = grid.get_line(row, col, dir, dist)
            for hex_pos in line:
                if grid.hex_distance(pos, hex_pos) <= dist:
                    if hex_pos not in los_hexes and grid.grid[hex_pos[0]][hex_pos[1]]["accessible"]:
                        range_set.add(hex_pos)
                    if not grid.grid[hex_pos[0]][hex_pos[1]]["accessible"]:
                        break  # Stop at obstruction
        # Add adjacent hexes not on LOS lines to fill gaps, respecting obstructions
        for hex_pos in all_hexes:
            if hex_pos not in los_hexes and hex_pos not in range_set:
                line_to_hex = grid.get_line_between(pos[0], pos[1], hex_pos[0], hex_pos[1])
                blocked = False
                for intermediate in line_to_hex[1:]:
                    if not grid.grid[intermediate[0]][intermediate[1]]["accessible"]:
                        blocked = True
                        break
                if not blocked:
                    range_set.add(hex_pos)
