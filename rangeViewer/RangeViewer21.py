...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    ...
    if pattern == "Line of Sight":
        ...
        # Define range using all six bisecting lines, stopping at obstructions
        range_set = set()
            ...
            for hex_pos in line:
                if grid.hex_distance(pos, hex_pos) <= dist:
                    if not grid.grid[hex_pos[0]][hex_pos[1]]["accessible"]:
                        break  # Stop at obstruction, only exclude hexes beyond
                    if hex_pos not in los_hexes:
                        range_set.add(hex_pos)
        # Final exclusion of LOS hexes (safety net)
