...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    ...
    if pattern == "Line of Sight":
        ...
        # Build range from all six bisecting lines with alternating steps
        range_set = set()
            ...
            if angle == 90:  # Straight east
                ...
                while step < dist:
                    ...
                    if (0 <= new_r < grid.rows and 0 <= new_c < grid.cols and 
                        ...
                        break
            else:  # Alternating steps for 30°, 150°, 210°, 330°
                row_steps = [-1, -2] if angle in (30, 330) else [1, 2]  # Up or down
                col_steps = [1, 2] if angle in (30, 150) else [-1, -2]  # Right or left
                while step < dist:
                    step += 1
                    dr = row_steps[step % 2] * (step + 1) // 2
                    dc = col_steps[step % 2] * (step + 1) // 2
                    if angle == 30:
                        new_r, new_c = row + dr, col + dc
                    elif angle == 150:
                        new_r, new_c = row + dc, col + dr
                    elif angle == 210:
                        new_r, new_c = row + dc, col - dr
                    elif angle == 330:
                        new_r, new_c = row + dr, col + dc
