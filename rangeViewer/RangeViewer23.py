...
# Adjusted bisecting directions based on your hexes
...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    ...
    if pattern == "Line of Sight":
        ...
        range_set = set()
        for i, dir in enumerate(BISECTING_DIRECTIONS):
            # Adjust step size for 90° and 270° (every 2 hexes)
            step_multiplier = 2 if i in (1, 4) else 1  # 90° and 270° need double steps
            line = []
            r, c = row, col
            for step in range(dist + 1):
                dr, dc = dir[0] * step_multiplier, dir[1] * step_multiplier
                new_pos = (r + dr * step, c + dc * step)
                if (0 <= new_pos[0] < grid.rows and 0 <= new_pos[1] < grid.cols and 
                    grid.hex_distance(pos, new_pos) <= dist):
                    line.append(new_pos)
                else:
                    break
            print(f"Direction {i*60+30}° {dir}: {line}")
            for hex_pos in line:
