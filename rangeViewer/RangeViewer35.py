...
def get_hex_direction(pos, target):
    """Calculate the approximate direction from pos to target in hex grid (30°, 90°, 150°, 210°, 270°, 330°)."""
    r1, c1 = pos
    r2, c2 = target
    dr = r2 - r1
    dc = c2 - c1
    
    # Handle zero difference (same hex, shouldn't happen in range_set)
    if dr == 0 and dc == 0:
        return DIRECTIONS[0]  # Default to 0° (arbitrary)
    
    # Approximate direction based on row/col differences
    if dr == 0:
        return DIRECTIONS[1] if dc > 0 else DIRECTIONS[4]  # 90° (E) or 270° (W)
    elif dc == 0:
        # Vertical movement isn't exact in hex, but we'll approximate
        return DIRECTIONS[0] if dr < 0 else DIRECTIONS[3]  # 0° (N) or 180° (S) approx
    else:
        # Diagonal directions (30°, 150°, 210°, 330°)
        # Use slope-like logic adjusted for hex grid
        if dr < 0 and dc > 0:
            return DIRECTIONS[0]  # 30° (NE)
        elif dr > 0 and dc > 0:
            return DIRECTIONS[2]  # 150° (SE)
        elif dr > 0 and dc < 0:
            return DIRECTIONS[3]  # 210° (SW)
        elif dr < 0 and dc < 0:
            return DIRECTIONS[5]  # 330° (NW)
    return DIRECTIONS[0]  # Fallback (shouldn't reach here)
...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    ...
    if pattern == "Mist/Shadow":
        ...
        # LOS check for each hex in range_set
        filtered_range = set()
        for target in range_set:
            direction = get_hex_direction(pos, target)
            path = grid.get_line(pos[0], pos[1], direction, dist)
            blocked = False
            for hex_pos in path[1:]:  # Skip starting pos
                if hex_pos == target:
                    break
                if not grid.grid[hex_pos[0]][hex_pos[1]]["accessible"]:
                    ...
                    break
            if not blocked:
                filtered_range.add(target)
        
        print(f"Range before LOS filter: {range_set}")
        print(f"Range after LOS filter: {filtered_range}")
        range_set = filtered_range
    else:
        range_set = set()  # Placeholder for other patterns
