...
import math
from hexgrid import HexGrid, DIRECTIONS  # Import DIRECTIONS from hexgrid
...
instructions_label = UILabel(relative_rect=pygame.Rect(10, 400, 300, 30), 
...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    ...
    if pattern == "Line of Sight":
        ...
        # Get the six LOS lines
        los_hexes = set()
        row, col = pos
        for dir in DIRECTIONS:  # Use the imported DIRECTIONS
        ...
        # Mist/Shadow is all hexes minus LOS lines, respecting obstructions
        range_set = set()
        for hex_pos in all_hexes:
            if hex_pos not in los_hexes:
                # Check if path to hex_pos is blocked by an obstruction
                line_to_hex = grid.get_line_between(pos[0], pos[1], hex_pos[0], hex_pos[1])
                blocked = False
                for intermediate in line_to_hex[1:]:  # Skip start pos
