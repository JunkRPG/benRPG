...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    ...
    if pattern == "Line of Sight":
        ...
        for dir in BISECTING_DIRECTIONS:
            line = grid.get_line(row, col, dir, dist)
            print(f"Direction {dir}: {line}")  # Debug output
            for hex_pos in line:
                if grid.hex_distance(pos, hex_pos) <= dist:
                    if not grid.grid[hex_pos[0]][hex_pos[1]]["accessible"]:
                        break  # Stop at obstruction, exclude beyond
                    range_set.add(hex_pos)  # Add all hexes, filter LOS later
    ...
    for event in pygame.event.get():
        ...
        if not consumed_event:
            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = event.pos
                hex_pos = hex_grid.get_hex_at_pixel(pos[0], pos[1])  # Fixed: grid -> hex_grid
                if hex_pos:
                    if event.button == 1:
