...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    ...
    if pattern == "Line of Sight":
        ...
        for i, angle in enumerate(angles):
            line = []
            r, c = row, col
            col_steps = [1, 2] if angle in (30, 150) else [-1, -2] if angle in (210, 330) else [2] if angle == 90 else [-2]
            for step in range(1, dist + 1):
                if angle == 90:  # Straight east
                    ...
                    dc = col_steps[(step - 1) % 2] * ((step + 1) // 2)
                    dr = -1 if step % 2 == 1 else -2
                    new_r, new_c = r + dr * ((step + 1) // 2), c + dc
                elif angle == 150:  # SE
                    dc = col_steps[(step - 1) % 2] * ((step + 1) // 2)
                    dr = 1 if step % 2 == 1 else 2
                    new_r, new_c = r + dr * ((step + 1) // 2), c + dc
