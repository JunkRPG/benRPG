...
import pygame
import pygame_gui
from pygame_gui.elements import UIDropDownMenu, UILabel, UITextEntryLine, UICheckBox
import math
...
# Add some obstacles for LOS testing
hex_grid.grid[7][11]["accessible"] = False
hex_grid.grid[8][12]["accessible"] = False
hex_grid.grid[9][11]["accessible"] = False

# UI setup
range_distance_entry = UITextEntryLine(relative_rect=pygame.Rect(10, 50, 100, 30), manager=manager, initial_text="5")
...
include_pos_checkbox = UICheckBox(relative_rect=pygame.Rect(10, 130, 20, 20), text="Include Position", manager=manager)
exclude_adj_checkbox = UICheckBox(relative_rect=pygame.Rect(10, 160, 20, 20), text="Exclude Adjacent", manager=manager)

description_label = UILabel(relative_rect=pygame.Rect(10, 200, 250, 200), text="Custom range", manager=manager)
...
def calculate_range(grid, pos, distance, pattern, include_pos, exclude_adj):
    """Calculate the range based on user inputs."""
    ...
    if pattern == "Line of Sight":
        ...
        range_set = {hex_pos for hex_pos in grid.get_movement_range(pos, dist) 
                     if not grid.has_clear_line_of_sight(pos, hex_pos) and grid.grid[hex_pos[0]][hex_pos[1]]["accessible"]}
    else:
        range_set = set()
...
current_range = calculate_range(hex_grid, center_pos, range_distance_entry.get_text(), 
                                pattern_dropdown.selected_option, include_pos_checkbox.checked, 
                                exclude_adj_checkbox.checked)
...
while running:
    ...
    for event in pygame.event.get():
                ...
                if event.button == 1:
                    ...
                    if hex_pos:
                        ...
                        current_range = calculate_range(hex_grid, hex_pos, range_distance_entry.get_text(), 
                                                        ...
                                                        include_pos_checkbox.checked, exclude_adj_checkbox.checked)
                elif event.button == 3:
                    dragging = True
        ...
        # Update range on UI changes
        if event.type in (pygame_gui.UI_TEXT_ENTRY_CHANGED, pygame_gui.UI_DROP_DOWN_MENU_CHANGED, 
                          pygame_gui.UI_CHECKBOX_CHANGED):
            current_range = calculate_range(hex_grid, hex_grid.selected_hex or center_pos, 
                                            range_distance_entry.get_text(), 
                                            pattern_dropdown.selected_option, 
                                            include_pos_checkbox.checked, exclude_adj_checkbox.checked)
    ...
    # Define colors for HexGrid draw method
    colors = {
        'BLUE': BLUE,
    ...
    pattern = pattern_dropdown.selected_option
    movement_range = current_range if pattern == "Area Effect" else None
    attack_range = current_range if pattern in ("Line of Sight", "Melee", "Echo", "Multi Echo", "Perimeter", "Mist/Shadow") else None
